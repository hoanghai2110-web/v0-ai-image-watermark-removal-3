...
export async function trackReferral(referralCode: string, referredUserId: string): Promise<void> {
  const supabase = await createClient()
...
// Record affiliate earning when referred user makes a purchase
...
export async function getAffiliateStats(userId: string): Promise<{
  ...
  if (refError || earningsError) {
    console.error("[v0] Error fetching affiliate stats:", {
      refError,
      earningsError,
    })
    return {
